...
function c2dhNerLanguageAndEntityPairsToHistographEntityProperties(pairs) {
  ...
  const entity = {
    ...
    entity__ned_model: entityModel,
    entity__ned_id: entityId,
    languages,
  ...
  const metadata = get(matchedResource, 'metadata', {})

  Object.keys(metadata).forEach(metadataKey => {
    const entityKey = MetadataKeysMapping[metadataKey] || `metadata__${metadataKey}`
    entity[entityKey] = metadata[metadataKey]
  })
